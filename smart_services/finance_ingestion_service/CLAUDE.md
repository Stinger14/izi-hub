...
- **`app/api/`** — HTTP layer only. Route handlers catch `ValueError` from the service layer and turn it into a `422`; no other error translation happens here. The `/email` route is additionally guarded by the `verify_ingestion_api_key` dependency (`app/core/security.py`), which checks a bearer token (`INGESTION_INBOUND_API_KEY`, distinct from the outbound `FINANCE_INGESTION_API_KEY`) and returns `401` (missing/wrong token) or `500` (key not configured) before the handler ever runs.
...
- **Parsers** implement the `EmailParser` ABC (`can_parse` / `parse`) in `app/services/parsers/base.py`. `BankAlertParser` matches Spanish-language keywords (`consumo`, `compra`, `retiro`, `transferencia`, `tarjeta`) and RD$/DOP amount patterns; `BankAlertParserUsd` matches English keywords (`purchase`, `debit`, `withdrawal`, `deposit`, etc.) and `$`/`USD` amount patterns. `EmailIngestionService._select_parser` picks the first parser whose `can_parse` returns true; no match raises `ValueError("No parser available for this email")`, which the API layer turns into a `422`. Adding a new bank format means adding a new parser to `EmailIngestionService.parsers`, not branching inside the existing ones.
