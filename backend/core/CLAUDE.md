...
- Finance is a light "old money" theme (warm ivory surfaces, walnut text, eggplant primary) scoped to `.finance-shell` via `--fin-*` CSS vars in `app.css`. `CoreWeb.FinanceComponents.fin_card/1` is the shared surface (ivory card + hairline border + soft walnut shadow) — use it (or the `--fin-card`/`--fin-surface`/`--fin-border` tokens) for any new finance surface instead of hardcoding colors. The old dark `.glass` component is gone.
