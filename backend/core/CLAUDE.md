...
- `.glass` (`CoreWeb.FinanceComponents.glass/1`) is the shared glassmorphism wrapper (`backdrop-blur-xl` + gradient + border) used across finance surfaces. Anything meant to look "glassy" needs `backdrop-blur-xl` on itself, not just a semi-transparent background color — a translucent bg with no blur just lets whatever's behind it show through sharply.
